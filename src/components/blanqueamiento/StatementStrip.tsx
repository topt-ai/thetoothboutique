...
export default function StatementStrip() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      const words = gsap.utils.toArray('.blanq-statement-word');
  ...
  const line1 = 'Antes salías del consultorio igual que entraste.'.split(' ');
  const line2 = 'Ahora sales en 10 minutos con otra sonrisa.'.split(' ');
    ...
    <section ref={sectionRef} className="w-full bg-dark py-[80px] px-8 md:px-[8%] text-center">
      <div className="max-w-4xl mx-auto">
        <div className="font-display italic text-[clamp(28px,3vw,48px)] text-white/40 font-normal leading-[1.2] mb-[28px]">
          {line1.map((word, i) => (
            <span key={i} className="blanq-statement-word inline-block mr-[0.25em]">
              {word}
            </span>
          ))}
        </div>

        <div className="w-full max-w-[160px] h-[1px] bg-white/[0.08] mx-auto mb-[28px]"></div>
